...
class IPhoneADBFast:
    ...
    def _video_worker(self) -> None:
        while not self._stop.is_set():
            ...
            try:
                ...
                # Unbuffered file-like wrapper around the raw TCP H264 stream.
                stream_file = sock.makefile("rb", buffering=0)

                # A slightly larger probe than the ffplay ultra-low-latency
                # example makes startup more robust while remaining quick.
                container = av.open(
                    stream_file,
                    mode="r",
                    format="h264",
                ...
                for packet in container.demux(video_stream):
                    ...
                    for frame in packet.decode():
                        ...
                        # OpenCV-compatible BGR ndarray.
                        image = frame.to_ndarray(format="bgr24")
                        now = time.perf_counter()

                        with self._frame_cond:
                            # Important: NO queue. Old frame is overwritten.
                            ...
                            if elapsed >= 1.0:
                                self._video_fps = (
                                    self._fps_window_frames / elapsed
                                )
                                self._fps_window_frames = 0
    ...
    def wait_first_frame(self, timeout: float = 5.0) -> Tuple[np.ndarray, int]:
