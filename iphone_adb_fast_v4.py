...
class IPhoneADBFast:
    ...
    def __init__(
        ...
        # Input RPC is intentionally isolated from the UI/video thread.
        # XCTest tap completion can take 100-300 ms; callers should use
        # tap_async()/tap_frame_async() when the display must not block.
    ...
    def tap(self, x: float, y: float) -> Any:
        """Synchronous tap. Waits for the DeviceKit/XCTest RPC response."""
        return self.rpc(
            "device.io.tap",
            {"x": float(x), "y": float(y)},
        )

    def tap_async(self, x: float, y: float) -> Future:
        """
        Non-blocking tap.

        Returns a Future immediately. The RPC/XCTest completion runs on the
        dedicated input worker, so OpenCV/video processing does not freeze.
        """
    ...
    def tap_frame(
        ...
        """Synchronous tap using coordinates in the decoded video frame."""
    ...
    def tap_frame_async(
        ...
        """
        Non-blocking frame-coordinate tap.

        Coordinate conversion is done immediately, then the DeviceKit RPC is
        sent on the dedicated input worker.
        """
        dx, dy = self.frame_to_device(x, y, frame)
        return self.tap_async(dx, dy)
    ...
    def close(self) -> None:
        # Stop accepting new async input and finish the current command before
        # closing the shared RPC socket.
